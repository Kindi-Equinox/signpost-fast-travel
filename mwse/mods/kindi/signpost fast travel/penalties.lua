    ...
    if config.showStats then
        local msg = tes3.messageBox {
            message = string.format(
                "You've reached %s!\nJourney Statistics:\n\nTravel Type: %s\nTime Travelled: %.2f hours\nHealth Change: %.2f points\nFatigue Change: %.2f points\nGold lost: %d Gold\nDisease Acquired: %s\nSkill Increase: %s\n",
                CELL, travelType or "Normal", timePen, healthPen, fatiguePen, goldPen, disease:gsub("^%a", string.upper),
                skillIncrease),
